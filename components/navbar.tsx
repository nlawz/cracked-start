...
export function NavBar() {
    ...
    const lastPath = mounted ? `/${pathname.split("/").pop()}` : "";
    ...
    const links = [
        ...
        },
        {
            title: "Developers",
            cover: (
                <Link href="/resources" onClick={handleOnClick}>
                    <Image
                        alt="Engine"
                        src="/crack.png"
                        quality={100}
            ...
            children: [
                {
                    path: "https://github.com/nlawz",
                    title: "Open Source",
                    icon: <Github size={19} />,
                },
                {
                    path: "https://crack.build",
                    title: "Documentation",
                    icon: <LucideAnvil size={20} />,
                },
                {
    ...
    if (!mounted || pathname.includes("pitch")) {
        return null;
    }
    ...
    return (
      <header
        className={cn(
          "sticky mt-4 top-4 z-[9999] px-2 md:px-4 md:flex justify-center",
          pathname === "/" &&
            "transition duration-1s ease-in-out animate-header-slide-down-fade"
        )}
      >
        <nav className="border border-border px-4 flex items-center backdrop-filter backdrop-blur-3xl bg-[#181818] bg-opacity-20 h-[50px] z-[9999] rounded-xl">
          <ContextMenu>
            <ContextMenuTrigger>
              <Link href="/">
                <span className="sr-only">crack.build</span>
                <LogoIcon />
              </Link>
            </ContextMenuTrigger>

            <ContextMenuContent
              className="w-[200px] bg-[#fff] opacity-70 text-black"
              alignOffset={20}
            >
              <div className="divide-y">
                ...
                >
                  <LogoIcon />
                  <span className="font-medium text-sm pt-24">Copy Logo as SVG</span>
                </ContextMenuItem>
              </div>
            </ContextMenuContent>
          </ContextMenu>
              ...
              return (
                ...
                >
                  ...
                  {children && (
                    <div
                      className={cn(
                        "absolute top-[48px] w-[576px] -left-[1px] bg-[#fff] backdrop-blur-md opacity-70 flex h-0 group-hover:h-[250px] overflow-hidden transition-all duration-300 ease-in-out border-l-[1px] border-r-[1px] rounded-xl text-black",
                        hidden && "hidden"
                      )}
                    >
          ...
          <a
            className="text-sm font-medium pr-2 border-l-[1px] border-border pl-4 hidden md:block  transition-opacity hover:opacity-70 duration-200"
            href="/login"
          >
            Sign in
          </a>
